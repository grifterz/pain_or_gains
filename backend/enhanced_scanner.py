        ...
        if blockchain == "solana":
            ...
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    return {
                        "name": data.get("name", ""),
                        "symbol": data.get("symbol", token_address[:6]),
                        "decimals": data.get("decimals", 9)
                    }
        elif blockchain == "base":
            # For Base, we can use Etherscan compatible APIs
            url = f"{ETHERESCAN_API}/api?module=token&action=tokeninfo&contractaddress={token_address}&apikey={BASESCAN_API_KEY}"
            async with session.get(url) as response:
                if response.status == 200:
                    data = await response.json()
                    if data.get("status") == "1":
                        token_info = data.get("result", [])[0] if data.get("result") else {}
                        return {
                            "name": token_info.get("name", ""),
                            "symbol": token_info.get("symbol", token_address[2:8]),
                            "decimals": int(token_info.get("decimals", 18))
                        }
    except Exception as e:
        logger.error(f"Error fetching token info for {token_address}: {str(e)}")
    
    # Return a default if we couldn't get proper info
    return {
